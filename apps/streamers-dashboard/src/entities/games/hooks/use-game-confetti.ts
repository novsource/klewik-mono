...
export const useGameConfetti = () => {
  ...
  const startWinnerConfetti = () => {
    fireworksIntervalRef.current = runWinnerConfetti()
  }

  const startDropoutConfetti = () => {
    fireworksIntervalRef.current = runDropoutConfetti()
  }
  ...
  return { startWinnerConfetti, startDropoutConfetti, stopAllConfetti }
}
...
function startFireCardConfetti(options: FireCardConfettiOptions) {
