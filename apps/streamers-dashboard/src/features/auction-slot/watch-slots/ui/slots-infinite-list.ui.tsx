...
export type SlotsInfiniteListProps = Omit<FlexProps, 'children'> & {
  data: AuctionSlot[]
  children: (donation: AuctionSlot, virtualizedItem: VirtualizedItem) => ReactNode
  ...
  shadowVirtualListProps?: Omit<
    ShadowVirtualListProps<AuctionSlot>,
    'data' | 'count' | 'scrollElementRef'
  >
}

export const SlotsInfiniteList = (props: SlotsInfiniteListProps) => {
  const {
    className,
    data,
    limit = 15,
  ...
  return (
    <Flex className={cn('h-full w-full', className)} {...restProps}>
      <ShadowVirtualList
        ...
        {...shadowVirtualListProps}
      >
        {isShouldShowEmptyContent && <EmptyAuctionSlotsList />}
...
function EmptyAuctionSlotsList() {
  return (
