...
export type UseUrlSearchParamOptions<Value> = {
...
export type UseUrlSearchParamsActionOptions = {
  /** The mode to use for writing to the URL */
  write?: 'push' | 'replace'
}

/** The use url search param return type */
export type UseUrlSearchParamReturn<Value> = {
...
export const createQueryString = (searchParams: URLSearchParams, mode: UrlSearchParamMode) => {
  ...
  if (mode === 'history')
    return `${searchParamsString ? `?${searchParamsString}` : ''}${hash}`
  if (mode === 'hash-params')
